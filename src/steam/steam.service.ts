...
export class SteamService {
  ...
  public async syncInventory(uuid: string, autoSync?: boolean, failed?: any) {
      ...
      if (sinceLastSync < this._steamInvCacheTime) {
        throw new HttpException(
          `You need to wait ${Math.floor(
            (this._steamInvCacheTime - sinceLastSync) / 1000 / 60,
          )} minutes to sync again.`,
    ...
    abstractItems.forEach((ai) => {
      ...
      const item = items.find(
        (item) => item.name === ai.name && item.tagSlot === ai.tagSlot,
      );
  ...
  private async _getAbstractSteamItems(
    ...
    while (moreItemsToFetch) {
      ...
      descriptions.push(
        ...response.data.descriptions.filter(
          (d: SteamItemDescription) =>
            !descriptions.find(
              (description) => description.classid === d.classid,
            ),
        ),
      );
    ...
    let abstractItems = descriptions.map((description) => {
      const amount = assets
        .filter((a) => a.classid === description.classid)
    ...
    abstractItems = [
      ...new Map(
        abstractItems.map((ai) => [JSON.stringify([ai.name, ai.tagSlot]), ai]),
      ).values(),
    ];
  ...
  private async _updateInventory(
    ...
    newInventoryItems: { item: Item; amount: number }[],
  ) {
    ...
    for (const newInventoryItem of newInventoryItems) {
      if (
        updatedInventoryItems.some(
          (ii) => ii.item.id === newInventoryItem.item.id,
        )
      ) {
    ...
    const iiToRemove: InventoryItem[] = updatedInventoryItems.filter(
      (uii) => !newInventoryItems.some((ii) => ii.item.id === uii.item.id),
    );
  ...
  public async getFriends(uuid: string) {
    ...
    const steamId = await this._steamFetcherService.getSteamProfileId(
      user.steamProfileLink,
    );

    const friendIds = (
      await this._steamFetcherService.getSteamFriendIds(steamId)
    ).data.friendslist.friends.map((friend) => friend.steamid);
    let friendNames: string[] = [];

    let doneFetchingFriends = false;
    while (!doneFetchingFriends) {
      const request = await this._steamFetcherService.getSteamNamesfromIds(
        friendIds.splice(0, 100),
      );
      const playerNames = request.data.response.players.map(
    ...
    const ownPlayer = request.data.response.players
      .map((player) => player.personaname)[0]
      .substring(0, 20);
